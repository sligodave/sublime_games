...
class InARowGame(Game):
    def __init__(self, view=None, rows=6, cols=7, target=4, players=2):
        ...
        self.colors = [
            {'name': 'white', 'scope': 'white', 'background': '#ffffff', 'foreground': '#ffffff', 'caret': '#ffffff'},
            {'name': 'yellow', 'scope': 'yellow', 'background': '#ffec38', 'foreground': '#000000', 'caret': '#ffec38'},
            {'name': 'red', 'scope': 'red', 'background': '#ff3a2f', 'foreground': '#000000', 'caret': '#ff3a2f'},
            {'name': 'orange', 'scope': 'orange', 'background': '#ff7f00', 'foreground': '#000000', 'caret': '#ff7f00'},
            {'name': 'blue', 'scope': 'blue', 'background': '#385eff', 'foreground': '#000000', 'caret': '#385eff'},
            {'name': 'green', 'scope': 'green', 'background': '#30ec34', 'foreground': '#000000', 'caret': '#30ec34'},
            {'name': 'magenta', 'scope': 'magenta', 'background': '#dd5eff', 'foreground': '#000000', 'caret': '#dd5eff'},
        ]
    ...
    def get_next_cell(self, cell, direction):
        cell_index = {
            'up': lambda cell: cell.index - self.col_count if cell.index >= self.col_count else None,
            'down': lambda cell: cell.index + self.col_count if cell.index <= self.col_count * (self.row_count - self.col_count) else None,
            'left': lambda cell: cell.index - 1 if cell.index % self.col_count else None,
            'right': lambda cell: cell.index + 1 if cell.index % self.col_count < self.col_count - 1 else None,
            'up_left': lambda cell: cell.index - (self.col_count + 1) if self.get_next_cell(cell, 'up') and self.get_next_cell(cell, 'left') else None,
            'up_right': lambda cell: cell.index - (self.col_count - 1) if self.get_next_cell(cell, 'up') and self.get_next_cell(cell, 'right') else None,
            'down_left': lambda cell: cell.index + (self.col_count - 1) if self.get_next_cell(cell, 'down') and self.get_next_cell(cell, 'left') else None,
            'down_right': lambda cell: cell.index + (self.col_count + 1) if self.get_next_cell(cell, 'down') and self.get_next_cell(cell, 'right') else None,
        }[direction](cell)
...
class InARowPromptCommand(sublime_plugin.WindowCommand):
    def run(self, rows=6, cols=7, target=4, players=2, view_id=None):
        ...
        self.window.show_input_panel('rows-cols[-target[-players]]', initial, self.on_done, None, None)
        ...
        for separator in ['-', ' ', ',', '\t', ':']:
            ...
            if len(bits) == 4 and len([x for x in bits if x.isdigit()]) == 4:
                ...
                self.window.run_command(
                    'in_aRow_start',
                    {'rows': rows, 'cols': cols, 'target': target, 'players': players, 'view_id': self.view_id}
                )
                break
